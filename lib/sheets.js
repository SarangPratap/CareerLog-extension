...
var buildUniqueSheetTitle = function() {
  var now = new Date();
  ...
  return 'Careerlog ' + year + ' ' + y + m + d + '-' + hh + mm + ss;
};

// ─── CREATE SHEET ─────────────────────────────────────────────────────

var createJobTrackerSheet = async function(accessToken) {
  var title = buildUniqueSheetTitle();
...
var findApplicationRow = async function(spreadsheetId, company, role, accessToken) {
  ...
  for (var i = 1; i < rows.length; i++) {
    ...
    if (targetCompany && rowCompany) {
      companyMatch = rowCompany === targetCompany
        || rowCompany.indexOf(targetCompany) !== -1
        || targetCompany.indexOf(rowCompany) !== -1;
    }

    // Empty role from AI should not match every existing row.
    var roleMatch = false;
    if (targetRole && rowRole) {
      roleMatch = rowRole === targetRole
...
var normalizeJobUrlForMatch = function(url) {
  if (!url) return '';
  var value = String(url).trim();
  if (!value) return '';

  // Ignore protocol/case/trailing slash to improve deterministic matching.
  return value
    .replace(/^https?:\/\//i, '')
    .replace(/\/$/, '')
    .toLowerCase();
...
var findApplicationRowByJobUrl = async function(spreadsheetId, jobUrl, accessToken) {
  var target = normalizeJobUrlForMatch(jobUrl);
  if (!target) return null;

  var res = await sheetsRequest(spreadsheetId, 'GET', 'values/Applications!G:G', null, accessToken);
  var rows = (res && res.values) || [];

  for (var i = 1; i < rows.length; i++) {
    var rowUrl = normalizeJobUrlForMatch(rows[i][0] || '');
    if (rowUrl && rowUrl === target) return i + 1;
  }

  return null;
};

var findLatestCompanyRow = async function(spreadsheetId, company, accessToken) {
  var res = await sheetsRequest(spreadsheetId, 'GET', 'values/Applications!A:A', null, accessToken);
  var rows = (res && res.values) || [];
  var targetCompany = (company || '').toLowerCase().trim();
  ...
  for (var i = 1; i < rows.length; i++) {
    ...
    if (companyMatch) return i + 1;
  }

  return null;
};

var isLikelyUpdateEvent = function(jobData) {
  if (!jobData) return false;
  if (jobData.isUpdate === true) return true;
...
var normalizeIsoDateOrEmpty = function(value) {
  if (!value) return '';
...
var resolveAppliedDate = function(jobData, today) {
  var parsedAppliedDate = normalizeIsoDateOrEmpty(jobData && jobData.appliedDate);
  if (parsedAppliedDate) return parsedAppliedDate;

  // Prefer the actual email timestamp when AI does not extract appliedDate.
  var emailDate = normalizeIsoDateOrEmpty(jobData && jobData.sourceEmailDate);
  if (emailDate) return emailDate;

  // For update-only emails, leave blank rather than fabricating a date.
  if (isLikelyUpdateEvent(jobData)) return '';
...
var addApplicationRow = async function(spreadsheetId, jobData, accessToken) {
  ...
  var appliedDate = resolveAppliedDate(jobData, today);

  // Keep newest applications on top by inserting a row at index 1 (row 2, below headers).
  await sheetsRequest(spreadsheetId, 'POST', ':batchUpdate', {
    requests: [
      {
        insertDimension: {
  ...
  await sheetsRequest(spreadsheetId, 'PUT',
    'values/Applications!A2:H2?valueInputOption=USER_ENTERED',
    { values: [[
      jobData.company     || '',
      jobData.role        || '',
      appliedDate,
...
var processAndUpdateSheet = async function(jobData, spreadsheetId, accessToken) {
  var existingRow = await findApplicationRowByJobUrl(spreadsheetId, jobData.jobUrl, accessToken);
  if (!existingRow) {
    existingRow = await findApplicationRow(spreadsheetId, jobData.company, jobData.role, accessToken);
  }

  // If AI marks this as an update but role parsing is weak/missing, fall back to company-level match.
  if (!existingRow && isLikelyUpdateEvent(jobData)) {
    existingRow = await findLatestCompanyRow(spreadsheetId, jobData.company, accessToken);
  }
...
var sheetsRequest = async function(spreadsheetId, method, path, body, accessToken) {
  ...
  if (!res.ok) {
    ...
    if (res.status === 403) {
      var apiMsg = (errBody && errBody.error && errBody.error.message) || 'Permission denied';
      throw new Error(
        'Sheets permission denied (403) for sheet ' + spreadsheetId + '. ' +
        'Google account token does not have access to this spreadsheet. ' +
        'Use Settings > Reconnect Google with the account that owns the sheet, or share the sheet with the signed-in account. ' +
        'API says: ' + apiMsg
      );
    }
    ...
    if (res.status === 404) {
      throw new Error(
        'Sheet not found (404) for sheet ' + spreadsheetId + '. ' +
        'Check that the Sheet ID is correct and the sheet still exists.'
      );
    }
